import { fragmentImgs } from "../content/zone-fragment-data.js";
...
document.addEventListener("DOMContentLoaded", () => {
  const fragmentsZoneImgs = fragmentImgs[zone];
  var fragmentIndex = 1;

  fragmentsZoneImgs.forEach((fragments) => {
      ...
      p.innerText = `Project ${fragmentIndex}`;
      name.appendChild(p);
      ...
      fragments.forEach((fragment) => {
        ...
        img.alt = `Project Image ${fragmentIndex}`;
  ...
  document.querySelectorAll(".zone-explorer-ref").forEach((ref) => {
    ref.href = `./angry-bunny/narrative.html?zone=${zone}`;
  });
  ...
  function populateGallery() {
    ...
    imagesContainers.forEach((container) => {
      for (let j = 0; j < imagesPerProject; j++) {
        ...
        const img = document.createElement("img");
        img.src = `assets/zone-explorer/img${imageIndex}.jpg`;
